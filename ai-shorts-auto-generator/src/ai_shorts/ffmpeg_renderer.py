...
def mp4_status(project_dir: Path) -> dict[str, Any]:
    ...
    status = {
        ...
        "subtitle_sidecars": _subtitle_sidecars(subtitle_manifest),
    }
    if not ffmpeg_path:
...
def render_mp4_from_preview(project_id: str, project_dir: Path) -> dict[str, Any]:
    ...
    subtitle_manifest = read_json(project_dir / "renders" / "subtitles" / "subtitle_manifest.json", {})
    if subtitle_manifest.get("status") != "subtitles_ready":
        status = {
            "project_id": project_id,
            "ffmpeg_available": bool(find_ffmpeg()),
            "ffmpeg_path": find_ffmpeg() or "",
            "mp4_path": str(preview_dir / "preview.mp4"),
            "status": "subtitles_missing",
            "target_duration_sec": preview_manifest.get("target_duration_sec", 0),
            "source_timing_plan": preview_manifest.get("source_timing_plan", ""),
            "subtitle_sidecars": _subtitle_sidecars(subtitle_manifest),
            "next_step": "Generate and review SRT/VTT subtitles before MP4 rendering.",
        }
        write_json(preview_dir / "mp4_status.json", status)
        return status
    ...
    concat_manifest = _write_concat_manifest(preview_dir, preview_manifest.get("frames", []))
    command = [
        ffmpeg_path,
        "-y",
        "-f",
    ...
    status = {
        ...
        "target_duration_sec": preview_manifest.get("target_duration_sec", 0),
        "total_duration_sec": preview_manifest.get("total_duration_sec", preview_manifest.get("target_duration_sec", 0)),
        "source_timing_plan": preview_manifest.get("source_timing_plan", ""),
        "concat_manifest": str(concat_manifest),
        "subtitle_sidecars": _subtitle_sidecars(subtitle_manifest),
...
def _write_concat_manifest(preview_dir: Path, frames: list[dict[str, Any]]) -> Path:
    concat_path = preview_dir / "ffmpeg_concat_frames.txt"
    lines: list[str] = []
    for frame in frames:
        frame_path = str(frame.get("frame_png", "")).replace("\\", "/")
        if not frame_path:
            continue
        duration_sec = max(0.1, float(frame.get("duration_sec", 0) or (float(frame.get("duration_ms", 1000)) / 1000)))
        lines.append(f"file '{frame_path}'")
        lines.append(f"duration {duration_sec:.3f}")
    if frames:
        last_path = str(frames[-1].get("frame_png", "")).replace("\\", "/")
        if last_path:
...
def _subtitle_sidecars(subtitle_manifest: dict[str, Any]) -> dict[str, str]:
    return {
        "srt": str(subtitle_manifest.get("srt_path", "")),
        "vtt": str(subtitle_manifest.get("vtt_path", "")),
        "timing_plan": str(subtitle_manifest.get("source_timing_plan", "")),
    }
