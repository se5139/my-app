...
def summarize_project_gate(project_dir: Path) -> dict[str, Any]:
    ...
    metadata = read_json(package_dir / "metadata_quality_report.json", {})
    ...
    gates = {
        ...
        "metadata": metadata.get("status") == "metadata_ready",
        "render_export": render_export.get("status") == "ready_for_manual_upload",
        "final_upload": final_upload.get("status") == "final_upload_ready",
    }
    order = ["project_review", "compliance", "render_plan", "subtitles", "audio", "gif_preview", "mp4", "audio_mix", "subtitle_burn_optional", "thumbnail", "final_media", "metadata", "render_export", "final_upload"]
...
def _next_step(blocking_gate: str, render_export: dict[str, Any], final_upload: dict[str, Any]) -> str:
    ...
    if blocking_gate == "metadata":
        return "제목, 설명, 태그, 고정댓글 메타데이터 품질을 점검하고 승인하세요."
